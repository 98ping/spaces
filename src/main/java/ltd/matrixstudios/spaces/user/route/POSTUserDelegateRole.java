...
@Controller
public class POSTUserDelegateRole {

    @Autowired public UserRepository repository;

    @RequestMapping(value = {"/user/{id}/roles/delegate"}, method = {RequestMethod.POST})
    public String delegateRole(HttpServletRequest request, @PathVariable String id, @RequestBody String payload) {
        ...
        System.out.println("Added the role " + role.name() + " to the user " + id);
